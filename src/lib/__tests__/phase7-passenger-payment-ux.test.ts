...
const submissionMigration = readFileSync(
  join(
    process.cwd(),
    "supabase/migrations/20260901190100_phase7_automatic_payfast_submission.sql",
  ),
  "utf8",
);
...
describe("Phase 7 automatic passenger PayFast UX", () => {
  ...
  it("keeps unpaid new trips internal until trusted PayFast confirmation", () => {
    ...
    expect(submissionMigration).toContain("DROP TRIGGER IF EXISTS rides_payment_before_acceptance_trigger");
    expect(submissionMigration).toContain("DROP FUNCTION IF EXISTS private.enforce_payment_before_ride_acceptance");
  });

  it("promotes a paid draft to Requested only from the confirmed payment trigger", () => {
    expect(submissionMigration).toContain("CREATE OR REPLACE FUNCTION private.submit_paid_pending_ride");
  ...
  it("returns PayFast to the exact trip and waits for trusted ITN confirmation", () => {
    expect(createPayment).toContain("const returnUrl = `${appUrl}/app/trip/${intent.ride_id}?${returnQuery}`");
    expect(createPayment).toContain("const cancelUrl = `${appUrl}/app/trip/${intent.ride_id}?${cancelQuery}`");
  ...
  it("stages fare-increasing edits and applies them only after a confirmed PayFast adjustment", () => {
    ...
    expect(editMigration).toContain("v_amount_due := greatest(v_proposed_total - v_previous_total, 0)");
    expect(editMigration).toContain("CASE WHEN v_amount_due > 0.01 THEN 'awaiting_payment' ELSE 'applying' END");
  ...
  it("keeps admin payment information read-only instead of making it an admin gate", () => {
    ...
    expect(adminPaymentSummary).toContain("if (rolesLoading || !isAdmin || loading || !payment) return null");
