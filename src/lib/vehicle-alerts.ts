...
export function getVehicleAlerts(v: Pick<VehicleProfile,
...
>): VehicleAlert[] {
  ...
  if (due != null && due > 0) {
    ...
    else if (due - odo <= SERVICE_WARN_KM) alerts.push({ label: "Service due soon", severity: "warning" });
  }
  ...
  for (const [name, d] of docs) {
    ...
    else if (days <= DOC_WARN_DAYS) alerts.push({ label: `${name} expiring soon`, severity: "warning" });
  }
