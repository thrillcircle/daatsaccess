...
function PassengerBookingsPage() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      const { data: b } = await supabase
        ...
        .select(
          "id,booking_reference,service_type,status,start_at,end_at,requested_companion_count,estimated_total,quoted_total,deposit_amount,deposit_status,metadata,passenger_notes,created_at",
        )
  ...
  return (
    <AppShell title="My Trips" nav={nav}>
      ...
      {loading ? (
        ...
        <div className="rounded-2xl border bg-card p-6 text-center text-sm text-muted-foreground">
          ...
          <Link to="/app/passenger/book" className="text-primary underline">
            Start a booking
          </Link>
          .
        </div>
        ...
        <div className="space-y-3">
          {bookings.map((b) => {
            ...
            const nextAction = ((): string => {
              ...
              if (b.status === "active" || ride)
                return ride ? "Track your live trip." : "Trip is active.";
            ...
            return (
              ...
              >
                ...
                <dl className="mt-3 grid grid-cols-2 gap-x-3 gap-y-1.5 text-xs">
                  ...
                  {a.length ? (
                    <div className="col-span-2">
                      <dt className="text-muted-foreground">Assistance</dt>
                      <dd className="flex flex-wrap gap-1">
                        {a.map((x) => (
                          <Badge key={x.id} variant="outline" className="text-[10px]">
                  ...
                  <div>
                    ...
                    <dd>
                      {b.estimated_total != null ? formatZAR(Number(b.estimated_total)) : "—"}
                    </dd>
                  </div>
                    ...
                    <>
                      {b.end_at ? (
                        <div className="col-span-2">
                          ...
                          <dd>
                            {new Date(b.end_at).toLocaleDateString("en-ZA", {
                              dateStyle: "medium",
                            })}
                          </dd>
                        </div>
                      ...
                      <div>
                        ...
                        <dd>
                          {b.deposit_amount != null
                            ? `${formatZAR(Number(b.deposit_amount))} · ${b.deposit_status}`
                            : b.deposit_status}
                        </dd>
                      </div>
                      <div>
                        ...
                        <dd>
                          {comps.length
                            ? comps.map((c) => c.full_name).join(", ")
                            : `${b.requested_companion_count} requested`}
                        </dd>
                      </div>
                    </>
                  ) : null}
                  {ride ? (
                    <div className="col-span-2">
                      <dt className="text-muted-foreground">Ride status</dt>
                      <dd>{ride.status.replace("_", " ")}</dd>
                    </div>
                  ) : null}
                </dl>
                ...
                {b.service_type === "extended_journey" &&
                itinerary.some((i) => i.booking_id === b.id) ? (
                  <details className="mt-2 rounded-lg border bg-background/40 p-2 text-xs">
