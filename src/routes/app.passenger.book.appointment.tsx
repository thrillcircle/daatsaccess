...
function generateOccurrences(start: Date, rule: RecurrenceRule, max = MAX_GENERATED_OCCURRENCES): Date[] {
  const out: Date[] = [];
  const endTs = rule.end_date ? new Date(rule.end_date).getTime() + 24 * 60 * 60_000 : Number.POSITIVE_INFINITY;
  const limit = Math.min(max, rule.occurrences ?? max);

  if (rule.frequency === "weekly" || rule.frequency === "biweekly" || rule.frequency === "custom") {
    const days = rule.weekdays && rule.weekdays.length ? rule.weekdays.slice().sort() : [start.getDay()];
...
function BookAppointmentPage() {
  ...
  useEffect(() => {
    ...
    setEstimating(true);
    route({
      data: { originLat: pickupPt.lat, originLng: pickupPt.lng, destLat: facilityPt.lat, destLng: facilityPt.lng },
    })
      .then((r) => {
        if (cancelled) return;
  ...
  const apptValid = !!apptDate && !Number.isNaN(apptDate.getTime()) && apptDate.getTime() > Date.now() + 60_000;
  const transportEstimate = outboundKm != null ? estimatePrice(outboundKm) : null;
  ...
  function toggleWeekday(dow: number, on: boolean) {
    setWeekdays((prev) => (on ? Array.from(new Set([...prev, dow])).sort() : prev.filter((d) => d !== dow)));
  }
  ...
  async function createBookingFor(startAt: string, parentBookingId: string | null) {
    ...
    const journeyPattern =
      pattern === "dropoff" ? "one_way"
      : pattern === "wait_return" ? "wait_and_return"
      : pattern === "recurring" && parentBookingId == null ? "recurring"
      : "return";
    ...
    const { data: booking, error: bookingErr } = await supabase
      ...
      .insert({
        ...
        recurrence_rule: parentBookingId == null && recurrenceRule ? (recurrenceRule as unknown as never) : null,
      })
      .select()
      .single();
    ...
    if (pattern === "wait_return" && effectiveReturnPt) {
      ...
      items.push({
        ...
        notes: JSON.stringify({ facilityName: facilityName.trim(), expectedDurationMin: durationMinExpected }),
      });
      items.push({
        booking_id: booking.id,
        day_number: 1,
    ...
    const { error: itinErr } = await supabase.from("booking_itinerary_items").insert(items as never);
  ...
  async function onSubmit() {
    ...
    try {
      if (pattern === "recurring") {
        ...
        toast.success(`Recurring appointment series created (${previewDates.length} dates) — awaiting quote`);
      } else {
  ...
  return (
    <AppShell title="Access Appointment" nav={nav}>
      ...
      <section className="rounded-2xl border bg-card p-4 shadow-[var(--shadow-card)]">
        ...
        <div className="mt-3 grid gap-3">
          <div>
            ...
            <Input id="appt-facility" value={facilityName} onChange={(e) => setFacilityName(e.target.value)} placeholder="e.g. Sandton Mediclinic" />
          </div>
          <AddressAutocomplete id="appt-facility-addr" label="Facility address" value={facilityPt} onChange={setFacilityPt} />
          <div className="grid grid-cols-2 gap-3">
            <div>
              <Label htmlFor="appt-time">Appointment date & time</Label>
              <Input id="appt-time" type="datetime-local" min={localInputNow()} value={apptLocal} onChange={(e) => setApptLocal(e.target.value)} />
            </div>
            <div>
              <Label htmlFor="appt-arrive">Arrive how early? (min)</Label>
              <Input id="appt-arrive" type="number" min={0} max={120} value={arriveBeforeMin} onChange={(e) => setArriveBeforeMin(Number(e.target.value) || 0)} />
            </div>
          </div>
          <div>
            <Label htmlFor="appt-duration">Expected appointment duration (min)</Label>
            <Input id="appt-duration" type="number" min={5} step={5} value={durationMinExpected} onChange={(e) => setDurationMinExpected(Math.max(5, Number(e.target.value) || 0))} />
          </div>
        </div>
      </section>
      ...
      <section className="mt-3 rounded-2xl border bg-card p-4 shadow-[var(--shadow-card)]">
        ...
        >
          {(["dropoff", "dropoff_collect", "wait_return", "recurring"] as AppointmentPattern[]).map((p) => (
            <Label
              key={p}
              htmlFor={`appt-p-${p}`}
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="block text-xs text-muted-foreground">{APPOINTMENT_PATTERN_DESCRIPTION[p]}</span>
              </span>
            </Label>
          ))}
        </RadioGroup>
      ...
      <section className="mt-3 rounded-2xl border bg-card p-4 shadow-[var(--shadow-card)]">
        ...
        <div className="mt-3 space-y-3">
          <AddressAutocomplete id="appt-pickup" label="Pickup address" value={pickupPt} onChange={setPickupPt} enableCurrentLocation />
          {(pattern === "dropoff_collect" || pattern === "wait_return") ? (
            <>
              <Label htmlFor="appt-ret-same" className="flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm">
                <Checkbox
                  id="appt-ret-same"
              ...
              {!returnSameAsPickup ? (
                <AddressAutocomplete id="appt-return" label="Return address" value={returnPt} onChange={setReturnPt} />
              ) : null}
            </>
          ) : null}
        </div>
      ...
      <section className="mt-3 rounded-2xl border bg-card p-4 shadow-[var(--shadow-card)]">
        ...
        <RadioGroup value={bookFor} onValueChange={(v) => setBookFor(v as "self" | "other")} className="mt-3 grid grid-cols-2 gap-2">
          <Label htmlFor="appt-bf-self" className="flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm">
            <RadioGroupItem id="appt-bf-self" value="self" /> Myself
          </Label>
          <Label htmlFor="appt-bf-other" className="flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm">
        ...
        <div className="mt-3 grid gap-3">
          <div>
            ...
            <Input id="appt-trav-name" value={travellerName} onChange={(e) => setTravellerName(e.target.value)} />
          </div>
          <div>
            <Label htmlFor="appt-trav-phone">Traveller phone</Label>
            <Input id="appt-trav-phone" value={travellerPhone} onChange={(e) => setTravellerPhone(e.target.value)} />
          </div>
          {bookFor === "other" ? (
            <div>
              <Label htmlFor="appt-trav-rel">Relationship to traveller</Label>
              <Input id="appt-trav-rel" value={relationship} onChange={(e) => setRelationship(e.target.value)} />
            </div>
          ) : null}
        </div>
      </section>
      ...
      <section className="mt-3 rounded-2xl border bg-card p-4 shadow-[var(--shadow-card)]">
        ...
        <p className="text-xs text-muted-foreground">How many trained companions should accompany the traveller?</p>
      ...
      <section className="mt-3 rounded-2xl border bg-card p-4 shadow-[var(--shadow-card)]">
        ...
        <div className="mt-3 grid gap-2">
          {ASSISTANCE_OPTIONS.filter((o) => o.code !== "other").map((opt) => (
            <Label key={opt.code} htmlFor={`appt-aa-${opt.code}`} className="flex cursor-pointer items-start gap-3 rounded-lg border p-3 text-sm">
              <Checkbox
                id={`appt-aa-${opt.code}`}
        ...
        <div className="mt-3">
          ...
          <Textarea id="appt-other-support" value={otherInstructions} onChange={(e) => setOtherInstructions(e.target.value)} />
        </div>
      </section>

      <section className="mt-3 rounded-2xl border bg-card p-4 shadow-[var(--shadow-card)]">
        <Label htmlFor="appt-notes">Operational notes for the team (optional)</Label>
        <Textarea id="appt-notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="e.g. wheelchair-accessible entrance on the south side" />
      </section>
      ...
      {pattern === "recurring" ? (
        <section className="mt-3 rounded-2xl border bg-card p-4 shadow-[var(--shadow-card)]">
          ...
          <p className="text-xs text-muted-foreground">
            We'll generate up to {MAX_GENERATED_OCCURRENCES} dates. Our team confirms each before any ride is dispatched.
          </p>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            ...
            {frequency === "custom" || frequency === "monthly" ? (
              <div>
                <Label htmlFor="appt-interval">Interval ({frequency === "monthly" ? "months" : "weeks"})</Label>
                <Input id="appt-interval" type="number" min={1} max={12} value={interval} onChange={(e) => setInterval(Math.max(1, Number(e.target.value) || 1))} />
              </div>
            ) : null}
          </div>
          ...
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div>
              ...
              <RadioGroup value={endMode} onValueChange={(v) => setEndMode(v as "date" | "count")} className="mt-1 grid grid-cols-2 gap-2">
                <Label htmlFor="appt-em-count" className="flex cursor-pointer items-center gap-2 rounded-lg border p-2 text-xs">
                  <RadioGroupItem id="appt-em-count" value="count" /> # of times
                </Label>
                <Label htmlFor="appt-em-date" className="flex cursor-pointer items-center gap-2 rounded-lg border p-2 text-xs">
            ...
            {endMode === "count" ? (
              <div>
                <Label htmlFor="appt-occ">Number of occurrences (max {MAX_GENERATED_OCCURRENCES})</Label>
                <Input id="appt-occ" type="number" min={1} max={MAX_GENERATED_OCCURRENCES} value={occurrences} onChange={(e) => setOccurrences(Math.max(1, Math.min(MAX_GENERATED_OCCURRENCES, Number(e.target.value) || 1)))} />
              </div>
            ) : (
              <div>
                <Label htmlFor="appt-end">End date</Label>
                <Input id="appt-end" type="date" value={recurEnd} onChange={(e) => setRecurEnd(e.target.value)} />
              </div>
            )}
          </div>
          ...
          {previewDates.length ? (
            ...
            <p className="mt-2 text-xs text-muted-foreground">Set frequency, weekdays and end condition to preview dates.</p>
          )}
        </section>
      ) : null}
      ...
      {!apptValid && apptLocal ? (
        <p className="mt-2 text-center text-xs text-muted-foreground">Appointment must be at least 1 hour from now.</p>
      ) : null}
    </AppShell>
  );
}
